...
/// Score at or below which a map cell is treated as changed when deriving
/// regions. Distinct from `minScore`, which decides whether the *pair* matches:
/// a pooled score can sit above the match floor while individual windows still
/// hold the change worth describing.
const DEFAULT_REGION_FLOOR: f64 = 0.99;

/// Interpret what changed between two images, using this crate's local score
/// map to locate the regions.
///
/// The map is coarse — its grid is the metric's window stride, and `ms-ssim`
/// reports at a pyramid scale — so the boxes are blocky. The statistics are
/// not: `blazediff-interpret` refines every box against the source pixels
/// before measuring, so `diffCount`, shape and colour analysis stay per-pixel.
///
/// `regionFloor` sets the per-window threshold (default 0.99).
#[napi]
pub fn interpret(
    base_path: String,
    compare_path: String,
    ...
    interpret_images(image1, image2, options, region_floor)
}

/// Buffer-input counterpart to [`interpret`].
#[napi]
...
fn interpret_images(
    ...
    let regions = blazediff_interpret::regions_from_score_map(
        &outcome.map,
        outcome.map_width,
        outcome.map_height,
        image1.width,
        image1.height,
        region_floor.unwrap_or(DEFAULT_REGION_FLOOR) as f32,
    );

    let result = blazediff_interpret::interpret_regions(&image1, &image2, &regions)
