...
# scripts/build-napi.sh sets them and execs this:
#
#   NAPI_CRATE       cargo package name          e.g. blazediff-ssim
#   NAPI_LIB         [lib] name                  e.g. blazediff_ssim
#   NAPI_ARTIFACT    dist basename prefix        e.g. blazediff-ssim
#   NAPI_NODE        filename in the package     e.g. blazediff_ssim.node
#   NAPI_PKG_PREFIX  platform-package family     e.g. ssim-native
: "${CRATE_DIR:?CRATE_DIR must be set before running build-napi.sh}"
: "${NAPI_CRATE:?NAPI_CRATE must be set}"
: "${NAPI_LIB:?NAPI_LIB must be set}"
: "${NAPI_ARTIFACT:?NAPI_ARTIFACT must be set}"
: "${NAPI_NODE:?NAPI_NODE must be set}"
: "${NAPI_PKG_PREFIX:?NAPI_PKG_PREFIX must be set}"

# Only blazediff links pyo3, so only it can be contaminated by a python build.
NAPI_CHECK_PYTHON="${NAPI_CHECK_PYTHON:-false}"
...
# can't overwrite each other's lib${NAPI_LIB}.so. Both writes land at
# target/<triple>/release/lib${NAPI_LIB}.so; sharing it means the last writer
...
check_no_python_symbols() {
    [[ "$NAPI_CHECK_PYTHON" == "true" ]] || return 0
    local file="$1"
...
build_native_napi() {
    ...
    RUSTFLAGS="-C target-cpu=native" cargo build --release --features napi --lib -p "$NAPI_CRATE"
    ...
    local src="$TARGET_DIR/release/${lib_prefix}${NAPI_LIB}${lib_ext}"
    local dst="$DIST_DIR/${NAPI_ARTIFACT}-${os}-${arch}.node"
    if [[ -f "$src" ]]; then
        cp "$src" "$dst"
...
build_napi_target() {
    ...
    local output_name="${NAPI_ARTIFACT}-${friendly}.node"
    ...
    if needs_xwin "$target"; then
        ...
        ( cd "$WORKSPACE_DIR" && DOCKER_DEFAULT_PLATFORM=linux/amd64 RUSTFLAGS="$flags" \
            ...
            -p "$NAPI_CRATE" --features napi --lib )
    else
        RUSTFLAGS="$flags" cargo build --release --target "$target" --features napi --lib -p "$NAPI_CRATE"
    fi
    ...
    local src="$TARGET_DIR/$target/release/${lib_prefix}${NAPI_LIB}${lib_ext}"
...
sync_napi_to_packages() {
    ...
    for binary in "${BUILT_NODES[@]}"; do
        ...
        # Strip the artifact prefix and .node suffix to recover the friendly
        # name the file was built under, then map it back to a triple.
        local friendly="${name#"${NAPI_ARTIFACT}-"}"
        friendly="${friendly%.node}"
        local target=""
        case "$friendly" in
