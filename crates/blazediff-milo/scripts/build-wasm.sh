...
# @blazediff/milo-wasm. Single target, no cross-compilation matrix: wasm
# is wasm.
#
...
# The copies stay in step by construction: WASM_BINDGEN_VERSION must match the
...
OUT_DIR="$PACKAGES_DIR/milo-wasm/wasm"
WASM_FILE="$TARGET_DIR/wasm32-unknown-unknown/release/blazediff_milo.wasm"

echo "Building blazediff-milo for wasm32-unknown-unknown..."
...
if command -v wasm-opt &> /dev/null; then
    ...
    wasm-opt -O3 \
        ...
        --enable-multivalue \
        -o "$OUT_DIR/blazediff_milo_bg.wasm" "$OUT_DIR/blazediff_milo_bg.wasm"
fi
