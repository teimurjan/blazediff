...
pub fn load_jpeg<P: AsRef<Path>>(path: P) -> Result<Image, ImageError> {
    let file = File::open(path.as_ref())?;
    let file_data = unsafe { Mmap::map(&file)? };
    decode_jpeg(&file_data)
}

pub fn decode_jpeg(file_data: &[u8]) -> Result<Image, ImageError> {
    unsafe {
        ...
        if tj3DecompressHeader(handle, file_data.as_ptr(), file_data.len()) != 0 {
            return Err(ImageError::Jpeg(get_tj_error(handle)));
        }
        ...
        if width == 0 || height == 0 {
            return Err(ImageError::Jpeg("Invalid JPEG dimensions".into()));
        }
...
pub fn save_jpeg<P: AsRef<Path>>(image: &Image, path: P, quality: u8) -> Result<(), ImageError> {
    unsafe {
