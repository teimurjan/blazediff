...
pub fn load_qoi<P: AsRef<Path>>(path: P) -> Result<Image, ImageError> {
    let file = File::open(path.as_ref())?;
    let file_data = unsafe { Mmap::map(&file)? };
    decode_qoi(&file_data)
}

pub fn decode_qoi(file_data: &[u8]) -> Result<Image, ImageError> {
    let (header, pixels) =
        qoi::decode_to_vec(file_data).map_err(|e| ImageError::Qoi(e.to_string()))?;
...
pub fn save_qoi<P: AsRef<Path>>(image: &Image, path: P) -> Result<(), ImageError> {
    let encoded = qoi::encode_to_vec(&image.data, image.width, image.height)
        .map_err(|e| ImageError::Qoi(e.to_string()))?;
