const fs = require("fs");
const a = JSON.parse(
  fs.readFileSync("packages/benchmark/pixelmatch_binary.json", "utf8")
);
const b = JSON.parse(
  fs.readFileSync("packages/benchmark/blazediff_binary.json", "utf8")
);
...
const allRows = [head, ...rows, ["**TOTAL**", `**${pxTotal.toFixed(2)}ms**`, `**${bzTotal.toFixed(2)}ms**`, `**${savedTotal.toFixed(2)}ms**`, `**${pctTotal.toFixed(1)}%**`]];
