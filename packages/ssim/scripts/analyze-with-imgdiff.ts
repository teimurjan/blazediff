...
function getImagePairs(dir: string): Array<{ name: string; img1: string; img2: string }> {
	if (!existsSync(dir)) {
		return [];
	}
...
): Record<string, { vsOriginal: number[]; vsFast: number[]; vsWeber: number[] }> {
	return {
		hitchMean: {
			vsOriginal: results.map((r) => r.timings.ssimjsOriginal / r.timings.hitchhikersMean),
			vsFast: results.map((r) => r.timings.ssimjsFast / r.timings.hitchhikersMean),
			vsWeber: results.map((r) => r.timings.ssimjsWeber / r.timings.hitchhikersMean),
		},
		ssim: {
		...
		ssimjsOriginal: {
			...
			vsFast: results.map((r) => r.timings.ssimjsFast / r.timings.ssimjsOriginal),
			vsWeber: results.map((r) => r.timings.ssimjsWeber / r.timings.ssimjsOriginal),
		},
		ssimjsFast: {
			vsOriginal: results.map((r) => r.timings.ssimjsOriginal / r.timings.ssimjsFast),
			vsFast: results.map(() => 1.0),
			vsWeber: results.map((r) => r.timings.ssimjsWeber / r.timings.ssimjsFast),
		},
		ssimjsWeber: {
			vsOriginal: results.map((r) => r.timings.ssimjsOriginal / r.timings.ssimjsWeber),
...
function generateMarkdownReport(
	...
	const avgSsimTime = results.reduce((a, r) => a + r.timings.ssim, 0) / results.length;
...
Hitchhiker's SSIM achieves **${corrHitchMean.toFixed(3)} correlation** with MATLAB reference and is **${hitchMeanSpeedups.vsWeber.toFixed(1)}x faster** than ssim.js (Weber).

| Implementation | Avg Time | vs Original | vs Fast | vs Weber | Correlation |
|----------------|----------|-------------|---------|----------|-------------|
| **Hitchhiker's (mean)** | ${avgHitchMeanTime.toFixed(1)}ms | ${hitchMeanSpeedups.vsOriginal.toFixed(1)}x | ${hitchMeanSpeedups.vsFast.toFixed(1)}x | ${hitchMeanSpeedups.vsWeber.toFixed(1)}x | ${corrHitchMean.toFixed(4)} |
| **SSIM (Gaussian)** | ${avgSsimTime.toFixed(1)}ms | ${ssimSpeedups.vsOriginal.toFixed(1)}x | ${ssimSpeedups.vsFast.toFixed(1)}x | ${ssimSpeedups.vsWeber.toFixed(1)}x | ${corrSsim.toFixed(4)} |
| **ssim.js (Weber)** | ${avgSsimjsWeberTime.toFixed(1)}ms | ${ssimjsWeberSpeedups.vsOriginal.toFixed(1)}x | ${ssimjsWeberSpeedups.vsFast.toFixed(1)}x | ${ssimjsWeberSpeedups.vsWeber.toFixed(1)}x | ${corrSsimjsWeber.toFixed(4)} |
| **ssim.js (Original)** | ${avgSsimjsOriginalTime.toFixed(1)}ms | ${ssimjsOriginalSpeedups.vsOriginal.toFixed(1)}x | ${ssimjsOriginalSpeedups.vsFast.toFixed(1)}x | ${ssimjsOriginalSpeedups.vsWeber.toFixed(1)}x | ${corrSsimjsOrig.toFixed(4)} |
| **ssim.js (Fast)** | ${avgSsimjsFastTime.toFixed(1)}ms | ${ssimjsFastSpeedups.vsOriginal.toFixed(1)}x | ${ssimjsFastSpeedups.vsFast.toFixed(1)}x | ${ssimjsFastSpeedups.vsWeber.toFixed(1)}x | ${corrSsimjsFast.toFixed(4)} |

## Score Statistics

| Implementation | Mean | StdDev | Range |
|----------------|------|--------|-------|
| **Hitchhiker's (CoV)** | ${hitchCovStats.mean.toFixed(3)} | ${hitchCovStats.stddev.toFixed(3)} | ${hitchCovStats.min.toFixed(3)} - ${hitchCovStats.max.toFixed(3)} |
| **Hitchhiker's (mean)** | ${hitchMeanStats.mean.toFixed(3)} | ${hitchMeanStats.stddev.toFixed(3)} | ${hitchMeanStats.min.toFixed(3)} - ${hitchMeanStats.max.toFixed(3)} |
| **SSIM (Gaussian)** | ${ssimStats.mean.toFixed(3)} | ${ssimStats.stddev.toFixed(3)} | ${ssimStats.min.toFixed(3)} - ${ssimStats.max.toFixed(3)} |
| **ssim.js (Weber)** | ${ssimjsWeberStats.mean.toFixed(3)} | ${ssimjsWeberStats.stddev.toFixed(3)} | ${ssimjsWeberStats.min.toFixed(3)} - ${ssimjsWeberStats.max.toFixed(3)} |
| **ssim.js (Fast)** | ${ssimjsFastStats.mean.toFixed(3)} | ${ssimjsFastStats.stddev.toFixed(3)} | ${ssimjsFastStats.min.toFixed(3)} - ${ssimjsFastStats.max.toFixed(3)} |
| **MATLAB** | ${matlabStats.mean.toFixed(3)} | ${matlabStats.stddev.toFixed(3)} | ${matlabStats.min.toFixed(3)} - ${matlabStats.max.toFixed(3)} |
...
|-------|------|-------------|--------|-------------|--------------|
${diffs.map((e) => `| ${e.name} | ${e.ssim.toFixed(4)} | ${e.hitch.toFixed(4)} | ${e.matlab.toFixed(4)} | ${Math.abs(e.ssim - e.matlab).toFixed(4)} | ${e.diff.toFixed(4)} |`).join("\n")}

## Interpretation

### Performance
- Hitchhiker's SSIM is **${hitchMeanSpeedups.vsWeber.toFixed(1)}x faster** than ssim.js (Weber)
- SSIM (Gaussian) is **${ssimSpeedups.vsOriginal.toFixed(1)}x faster** than ssim.js (Original)
- Average processing times: Hitchhiker's ${avgHitchMeanTime.toFixed(1)}ms, SSIM ${avgSsimTime.toFixed(1)}ms
...
### Accuracy
- **${corrHitchMean.toFixed(3)} correlation** with MATLAB validates algorithmic correctness
- SSIM (Gaussian) achieves near-perfect **${corrSsim.toFixed(4)} correlation** with MATLAB
...
- Applications where **${hitchMeanSpeedups.vsWeber.toFixed(1)}x speedup** over ssim.js (Weber) matters

**Use SSIM (Gaussian) for:**
- Cases requiring exact MATLAB compatibility (${corrSsim.toFixed(4)} correlation)
- Academic research requiring Gaussian weighting
- When ${ssimSpeedups.vsOriginal.toFixed(1)}x speedup over ssim.js (Original) is sufficient
...
async function main() {
	...
	for (const { name, img1, img2 } of pairs) {
		...
		const [image1, image2] = await Promise.all([
			transformer.transform(img1),
			transformer.transform(img2),
		]);
		...
		const ssimScore = ssim(image1.data, image2.data, undefined, image1.width, image1.height, {});
		...
		const ssimjsOriginalResult = ssimjs(imageData1, imageData2, { ssim: "original" });
