...
describe("Hitchhiker's SSIM - Comparison with MATLAB SSIM", () => {
	...
	it("should compare with MATLAB SSIM for images 2a vs 2b", async () => {
		...
		expect(hitchhikersMean).toBeGreaterThan(0.90);
		expect(hitchhikersMean).toBeLessThan(1.0);
		expect(hitchhikersCov).not.toBe(hitchhikersMean);
	});
...
describe("Hitchhiker's SSIM - Algorithm Correctness", () => {
	...
	describe("CoV pooling vs mean pooling", () => {
		it("should produce different scores with different pooling methods", () => {
			...
			const scoreCov = hitchhikersSSIM(img1, img2, undefined, width, height, { covPooling: true });
			const scoreMean = hitchhikersSSIM(img1, img2, undefined, width, height, { covPooling: false });
	...
	describe("window options", () => {
		it("should work with different window sizes", () => {
			...
			const score8 = hitchhikersSSIM(img1, img2, undefined, width, height, { windowSize: 8 });
			const score11 = hitchhikersSSIM(img1, img2, undefined, width, height, { windowSize: 11 });
			const score16 = hitchhikersSSIM(img1, img2, undefined, width, height, { windowSize: 16 });
		...
		it("should work with overlapping windows", () => {
			...
			const scoreNonOverlap = hitchhikersSSIM(img1, img2, undefined, width, height, {
				windowSize: 11,
				windowStride: 11,
			});

			// Overlapping (stride < windowSize)
			const scoreOverlap = hitchhikersSSIM(img1, img2, undefined, width, height, {
				windowSize: 11,
				windowStride: 5,
			});
