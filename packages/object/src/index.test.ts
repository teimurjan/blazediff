...
describe("diff", () => {
  ...
  describe("simple objects", () => {
    ...
    it("should handle multiple changes", () => {
      const result = diff({ a: 1, b: 2, c: 3 }, { a: 10, c: 3, d: 4 });
    ...
    it("should handle objects with undefined values", () => {
      const result = diff({ a: undefined, b: 1 }, { a: null, b: 1 });
      expect(result).toEqual([
        {
          type: DifferenceType.CHANGE,
          path: ["a"],
  ...
  describe("nested structures", () => {
    ...
    it("should handle arrays in objects", () => {
      const result = diff({ data: [1, 2, 3] }, { data: [1, 2, 4] });
      expect(result).toEqual([
        {
          type: DifferenceType.CHANGE,
          path: ["data", 2],
  ...
  describe("type changes", () => {
    ...
    it("should handle nested type changes", () => {
      const result = diff({ data: { value: 1 } }, { data: [1] });
      expect(result).toEqual([
        {
          type: DifferenceType.CHANGE,
          path: ["data"],
  ...
  describe("cycle detection", () => {
    it("should handle circular references with detectCycles enabled", () => {
      ...
      const changeInA = result.find(
        (d) => d.path.length === 1 && d.path[0] === "a"
      );
    ...
    it("should handle circular references in arrays", () => {
      ...
      const changeAt1 = result.find(
        (d) => d.path.length === 1 && d.path[0] === 1
      );
  ...
  describe("edge cases", () => {
    ...
    it("should handle empty string keys", () => {
      const result = diff({ "": "empty1", a: 1 }, { "": "empty2", a: 1 });
      expect(result).toEqual([
        {
          type: DifferenceType.CHANGE,
          path: [""],
    ...
    it("should handle unicode keys", () => {
      const result = diff({ "🔥": 1, 中文: 2 }, { "🔥": 10, 中文: 20 });
      expect(result).toHaveLength(2);
      expect(result).toContainEqual({
        type: DifferenceType.CHANGE,
        path: ["🔥"],
  ...
  describe("path accuracy", () => {
    ...
    it("should use numeric paths for array indices", () => {
      const result = diff({ arr: ["a", "b", "c"] }, { arr: ["a", "x", "c"] });
    ...
    it("should use string paths for object keys that look like numbers", () => {
      const result = diff({ "123": "value1" }, { "123": "value2" });
