...
import { loadPNG, savePNG } from "@blazediff/matcher";
...
describe("toMatchImageSnapshot", () => {
	...
	describe("threshold types", () => {
		it("should work with pixel threshold", async () => {
			const img = createTestImage(100, 100);

			await expect(img).toMatchImageSnapshot({
				method: "core",
				snapshotsDir: TEMP_DIR,
				snapshotIdentifier: "threshold-pixel",
				failureThreshold: 50,
				failureThresholdType: "pixel",
			});
		});

		it("should work with percent threshold", async () => {
			const img = createTestImage(100, 100);

			await expect(img).toMatchImageSnapshot({
				method: "core",
				snapshotsDir: TEMP_DIR,
				snapshotIdentifier: "threshold-percent",
				failureThreshold: 5,
