...
WebAssembly build of the BlazeDiff Rust algorithm for browsers, edge runtimes, and any wasm host. Same two-pass block algorithm as [`@blazediff/core-native`](/apis/core-native), compiled to `wasm32` with `v128` SIMD (`+simd128`). ~55% faster than [pixelmatch](https://github.com/mapbox/pixelmatch) on the same RGBA buffers — up to ~90% on 4K; diff counts agree with pixelmatch to within ~0.05%.
