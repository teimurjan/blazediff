...
Each `ChangeRegion` carries a `changeType`, `shape`, `position`, `confidence`, and the statistics behind them — colour delta, gradient/edge correlation, fill ratios, and the classifier's signals.
