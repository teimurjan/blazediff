---
title: Choosing a Metric - Pixel Diff vs SSIM vs GMSD vs MILO
description: Which image comparison metric to use for visual regression - exact pixel diffing, SSIM, MS-SSIM, Hitchhiker's SSIM, GMSD, or the learned MILO metric. Side-by-side comparison, thresholds, and how to score rendering changes without raw RGB thresholds.
---
...
rendering noise that pixel counting flags as failures. MILO goes one step
further and predicts where a person would *notice*, at a much higher cost.
...
| **MILO**              | Would a person notice          | Raw error, `0` = identical | Lower is better | Very high  | A second opinion on a failed diff, quality grades for reports |

Read the details: [what SSIM measures](/docs/metrics/ssim) ·
[how GMSD works](/docs/metrics/gmsd) ·
[what MILO adds](/docs/metrics/milo).
...
| A brand color changed                                        | Pixel diff. SSIM and GMSD are near-blind to it; MILO sees it |
| A failed diff on a photo or texture that nobody can see      | MILO, gate on `rawError`                          |
...
<Callout type="warning">
  **Both structural metrics are weak on color.** SSIM and GMSD work on luminance.
  ...
  color correctness matters, keep a pixel diff in the pipeline. MILO works on
  RGB and does count a hue change.
</Callout>
...
| MILO                 | `maxError` on `rawError`                         | `0.0005`; `0` for identical only |
...
MILO is not wired into the matcher or the CLI yet; call
[`@blazediff/milo-native`](/apis/milo-native) directly.
...
- MILO: a convolutional network at full resolution, **~0.4s for 1.6 MP on all cores**,
  2.5s on one. Not a per-screenshot gate for large suites
