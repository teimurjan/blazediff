...
Pass `interpret: true` to `compare()` or use the standalone `interpret()` function to get structured diff analysis — region detection, content-aware classification, severity scoring, and human-readable summaries.
...
if (!diff.match && diff.reason === 'pixel-diff') {
  console.log(diff.interpretation?.summary);
}

// HTML report
await compare('expected.png', 'actual.png', 'report.html', {
  outputFormat: 'html',
});
```
...
Change types: `Addition`, `Deletion`, `Shift`, `ContentChange`, `ColorChange`, `RenderingNoise`. Severity: Low (&lt;1%), Medium (1-10%), High (&gt;10%).
