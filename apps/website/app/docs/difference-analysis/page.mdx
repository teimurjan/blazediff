...
It is deterministic. No model, no weights, no network call. The whole pipeline
runs in the same binary as the diff, in both the Node binding
(`@blazediff/core-native`) and the WebAssembly build (`@blazediff/core-wasm`), so
it works server-side and in the browser.
...
Both bindings return the same shape: a `summary` string, a `regions[]` array with
`position`, `changeType` and `percentage` on each entry, and an overall
`severity`. The Node binding reads files; the wasm binding takes pre-decoded RGBA
buffers.

<Tabs items={['core-native (Node)', 'core-wasm (browser)']}>
  <Tabs.Tab>
...
```ts
import { initBlazediff, interpret } from "@blazediff/core-wasm";

await initBlazediff(); // loads the .wasm; call once

// Decode both images to RGBA Uint8Arrays (see the WebAssembly page)
...
When nothing changed, `regions` is empty and `summary` says so, in both bindings.
