...
Command-line tool for high-performance image comparison. Supports multiple algorithms including the native Rust binary (fastest), JavaScript pixel diff, GMSD, SSIM, MS-SSIM, Hitchhiker's SSIM, and structured diff interpretation.
...
- **Multiple Algorithms**: Pixel diff, GMSD, SSIM, MS-SSIM, Hitchhiker's SSIM, Interpret
...
<Tabs items={['core-native (default)', 'core', 'gmsd', 'ssim', 'msssim', 'hitchhikers-ssim', 'interpret']}>
  <Tabs.Tab>
...
Structured diff analysis — detects change regions and classifies them with content-aware change types (Addition, Deletion, Shift, ContentChange, ColorChange, RenderingNoise). Returns a structured summary grouped by change type.
...
```bash
blazediff-cli interpret image1.png image2.png --output report.html
```

#### Options

```bash
blazediff-cli interpret <image1> <image2> [options]
...
- **Summary** — structured overview grouped by change type
- **Severity** — Low (&lt;1%), Medium (1-10%), High (&gt;10%)
- **Regions** — each with position, change type, confidence, and percentage

If `--output` ends with `.html`, the command writes an interactive visual report instead of printing JSON to stdout.
...
```bash
$ blazediff-cli interpret reference.png test.png
{
  "summary": "Moderate visual change detected (1.87% of image, 12 regions).\nContent changed: 4 regions (bottom, center).\nContent added: 3 regions (right, bottom, bottom-left).\n...",
  "severity": "Medium",
  "diffPercentage": 1.87,
  "regions": [...]
}

$ blazediff-cli interpret reference.png test.png --compact
{
  "summary": "Moderate visual change detected (1.87% of image, 12 regions).\n...",
  "severity": "Medium",
  "diffPercentage": 1.87,
  "regions": [{ "position": "Bottom", "changeType": "Deletion", "confidence": 1.0, ... }]
}

$ blazediff-cli interpret reference.png test.png --output report.html
# writes report.html with side-by-side images and clickable region rows
```

<Callout type="info">
  **Same engine as `@blazediff/interpret-native`** — the CLI command uses the same Rust + SIMD binary under the hood, just wrapped for command-line use.
</Callout>
...
**Use `interpret` when:**
- You need to understand _what_ changed, not just _how much_
- You want semantic classification of diff regions
- You're building CI/CD reports with human-readable summaries
- You need region-level analysis (position, shape, severity)
