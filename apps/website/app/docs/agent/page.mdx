...
From Claude Code (or any agent that loads the [BlazeDiff skill](https://github.com/teimurjan/blazediff/blob/main/skill/blazediff/SKILL.md)):
...
- Prefer a stable, intent-revealing attribute. Add `data-blazediff-mask="<reason>"` to the component root in source and select on it:

  ```tsx
  <div data-blazediff-mask="report-carousel" className="...">
    ...
  </div>
  ```
...
cat <<'EOF' | blazediff-agent capture --stdin --mode baseline --json
[
  {"id": "agent", "url": "/agent", "mask": ["[data-blazediff-mask=\"report-carousel\"]"]},
