...
```json
{
  ...
  "auth": {
    "harness": ".blazediff/auth.js",
    "loginUrl": "http://127.0.0.1:3000/login"
  }
}
```

The `auth` block is added by `blazediff-agent auth init` and only needed for projects with auth-gated routes — see [Auth-protected routes](#auth-protected-routes) below.
...
```ts
{
  ...
  fullPage: boolean;
  auth: null | string;          // persona name; null = public route
}
```

## Auth-protected routes

Routes behind a login flow capture through a per-project harness. Credentials
live in environment variables — never in the harness file, the manifest, or
LLM context.

**One-time setup.** From your project root, with the dev server running:

```sh
blazediff-agent auth init --persona default --login-url http://127.0.0.1:3000/login
```

This opens a Playwright recorder. Log in once. When you close the window, the
agent post-processes the recording, swaps the email/password you typed for
`process.env.BLAZEDIFF_AUTH_<PERSONA>_EMAIL` / `..._PASSWORD`, and writes
`.blazediff/auth.js`. The file is short (~30 lines), reviewable, and meant to
be committed.

**Per-entry.** Mark an entry's `auth` field with the persona name (use
`"default"` for the common case):

```sh
blazediff-agent manifest add dashboard --url /dashboard --auth default
```

**At capture / check time.** Set the env vars before running:

```sh
export BLAZEDIFF_AUTH_DEFAULT_EMAIL='you@example.com'
export BLAZEDIFF_AUTH_DEFAULT_PASSWORD='hunter2'
blazediff-agent check
```

`check` fails fast if any persona referenced in the manifest is missing its
env vars, before any browser launches.

**Multiple personas.** Re-run `auth init --persona admin` to record another
flow into the same `.blazediff/auth.js` (the file is a single module taking
`persona` as an argument). Each persona maps to its own env-var pair.

**What's not supported in v0.2.** OAuth, SSO, magic links, MFA — the recorder
expects a password form. `storageState` reuse is not yet implemented; every
auth-gated capture re-runs the harness in a fresh browser context.
...
ships a hand-authored `.blazediff/auth.js` and committed baselines, so you can
clone the repo and run `pnpm --filter @blazediff/agent-auth-spa-example check`
to see the full flow pass 10/10.

## CI

In CI (`CI=1` or no TTY), only `check` is allowed. `init` / `capture` / `rewrite` / `reset` are explicitly blocked - authoring belongs at the developer's machine.
...
- run: npx blazediff-agent --cwd apps/website check --json
  env:
    # Only needed if any manifest entry has `auth: "<persona>"`.
    # Add one pair per persona referenced in the manifest.
